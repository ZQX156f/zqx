...
如上图所示，我们现在是以pacman作为己方。max操作是pacman推演的接下来可以行进的方向，但是它还需要去推演这一步操作之后幽灵会如何应对，幽灵这么应对之后，我pacman还有没有进一步对抗的空间，采取的行动。我采取了这个行动之后，幽灵又会如何狙击我............

我们在分析的时候，假设我们思考的步骤是接下来2步的操作，那么我们从**最底层出发来开始思考**。

**max层需要选择min结果中最大的**，如何理解呢？对手方的操作，对我造成的影响，我应该选择一个对我影响最小的。

**min层需要选择max结果中最小的**，如何理解呢？因为我们现在在做推演，我们需要预想到对方手想方设法知我于死地，所以虽然下层的max层操作可能会产生比较大的一个值，但是幽灵对手方肯定会阻止这个行为的发生，所以选择最小值。

通过上述这个描述，我们需要清楚，走接下来一步的得分以及对应的方向。

## α-β剪枝

节点数如果过多的时候，那么运算量、算力是呈现指数级别增长的，但是有时候有些节点其实完全没有必要再去处理分析了。那么这个时候我们便可以使用剪枝的方式将其抹除掉。

如果对于我们来说，我们通过查看树图，是可以看出来哪些是需要进行剪枝的，但是如果要求我们以一种公式化的形式将其展示出来，那么我们可能觉得展示不出来。α-β剪枝该算法的发明者通过在过程中引入了α-β这两个变量，巧妙地将这一过程以一种数学运算的过程展示了出来。后续我们判断是否需要剪枝，只需要带入这个公式即可得出来。





pacman游戏理论上来说其实也是可以使用α-β剪枝来对程序代码进行优化的，但是实际操作起来可能并不是特别的理想，为什么呢？

因为pacman行进过程中每个节点产生的数值基本上都是一样的，没有区分度，所以后续使用剪枝对于pacman游戏的提升效果很不显著。但是我们的目的其实并不是教大家如何去编写一个pacman游戏，而是基于pacman游戏的可视化窗口去介绍一些人工智能领域的基础算法。

![image-20240201104726807](image/image-20240201104726807.png)
