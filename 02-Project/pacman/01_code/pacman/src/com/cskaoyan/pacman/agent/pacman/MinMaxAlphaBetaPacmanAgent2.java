 ...
 * @Version V1.0
 **/
public class MinMaxAlphaBetaPacmanAgent2 implements Agent {
    @Override
    public Queue<Position> getPlan(GameStatus gameStatus) {
        ...
        //最终希望max返回的结果便是对应的方向和最大的分数，直接取出方向即可
         ScorePosition scorePosition = max(3, gameStatus, -90000000, 90000000);
         positions.offer(scorePosition.position);
        return positions;
    }

    //depth层级，就是表示的是思考接下来的几步操作；视野的宽度
    private ScorePosition max(int depth, GameStatus gameStatus, int alpha, int beta) {
        if(depth < 1){
            //结束了 todo
        ...
        for (Node item : aroundNodes) {
            ...
            int min = min(depth, gameStatusCopy,alpha, beta).score;
            stepScore += min;
            ...
            //更新alpha
            alpha = Math.max(alpha, stepScore);
        }

        return initState;
    }

    private ScorePosition min(int depth, GameStatus gameStatus, int alpha, int beta) {
        ...
        for (Node item : aroundNodes) {
            ...
            int max = max(depth, gameStatusCopy, alpha, beta).score;
            stepScore += max;
            //需要找到一个小值
            if(stepScore < initState.score){
                initState.score = stepScore;
